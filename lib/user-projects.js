...
module.exports.browse = async function({rl, us, username}) {
  await pagedList.browse({
    rl, us,
    getItems: async n => (await fetchUserProjects(username, n)).projects,
    title: `\x1b[34;1m${username}'s\x1b[0;1m projects\x1b[0m`,
    formatItem: p => `\x1b[33m${p.name}\x1b[0m`,
    pageCount: (await fetchUserProjects(username)).pageCount,
    handleItem: async p => {
...
function fetchUserProjects(username, pageNumber = 1) {
