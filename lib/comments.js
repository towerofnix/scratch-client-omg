...
module.exports.browse = async function({rl, us, pageType, pageId, pageObj = null, jumpTo = null}) {
  ...
  const { comments, jumpedComment } = await fetchComments(pageType, pageId, 1, jumpTo)
...
async function fetchComments(type, id, page = 1, jumpTo = null) {
  const comments = await (
    fetch(`${util.urls.siteAPI}/comments/${type}/${id}/?page=${page}`)
      .then(res => res.text())
  ...
  if (jumpTo) {
    ...
    if (jumpedComment) {
      ...
      if (comments.length) {
        ...
        return {comments, jumpedComment: null}
      }
    }
  } else {
    return {comments}
