...
module.exports.browse = async function({rl, us, id}) {
  ...
  while (!quit) {
    ...
    await util.choose({rl, us}, {
      ...
      // TODO: Normally, sending a new comment is "C". That behavior should
      // probably be moved to the comment browser instead. Anyways, right
      // now there's no way to send new top-level comments to a studio.
      C: {
