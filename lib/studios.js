...
module.exports.browse = async function({rl, us, id, jumpToComment = null}) {
  ...
  const showDescription = function() {
    if (studio.description) {
      ...
      console.log(util.wrap(studio.description))
      console.log('')
    }
  }
  ...
  const showComments = function(jumpTo = null) {
    return comments.browse({
      rl, us, pageType: 'gallery', pageId: id, pageObj: studio, jumpTo,
      commentsEnabled: studio.commentsEnabled
    })
  }
...
function fetchStudio(id, us) {
  return fetch(`${util.urls.newAPI}/studios/${id}`)
    .then(res => res.json())
    .then(data => {
      ...
      return fetchExtraStudioData(id, us).then(data => {
        Object.assign(studio, data)
      }).then(() => studio)
    })
}

function fetchExtraStudioData(id, us) {
  return fetch(`${util.urls.scratch}/studios/${id}/comments`, {
