...
const npmWrap = require('word-wrap')
...
module.exports.getAuthToken = function(us) {
  if (us.authToken) {
    return Promise.resolve(us.authToken)
  }
...
module.exports.wrap = function(string) {
  // Pre-wrap processing to trim out excessive whitespace. See issue #34 (e.g. project 261210105).
  string = (string
    // Replace whitespace-only lines with actually blank lines (just \n).
    .replace(/^[^\S\n]+$/gm, '')

    // Replace 4+ blank lines with a message showing that not all the lines are shown.
    .replace(/\n(\n{3,})\n/g, (match, group) => {
