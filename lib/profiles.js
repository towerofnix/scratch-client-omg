...
module.exports.browse = async function({rl, us, username, jumpToComment}) {
  ...
  while (!quit) {
    ...
    if (firstTime) {
      if (profile.aboutMe || profile.wiwo) {
        console.log('')
      }

      process.stdout.write('\x1b[1mAbout me:\x1b[0m')
      if (profile.aboutMe) {
        console.log('\n' + util.wrap(profile.aboutMe))
        console.log('')
      } else {
        console.log(' \x1b[2m(N/A)\x1b[0m')
        if (profile.wiwo) {
          console.log('')
        }
      }

      process.stdout.write('\x1b[1mWhat I\'m working on:\x1b[0m')
      if (profile.wiwo) {
