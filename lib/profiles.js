...
module.exports.browse = async function({rl, us}, profile) {
  if (profile.notFound) {
  ...
  while (!quit) {
    ...
    await util.choose({rl, us}, {
      ...
      f: profile.featuredProject ? {
        ...
        longcodes: ['featured'],
        action: async () => {
          await projects.browse({rl, us}, await projects.get(profile.featuredProject.id))
        }
      } : undefined,
      ...
      P: profile.projectCount ? {
        ...
        action: async () => {
          await userProjects.browse({rl, us}, profile.username)
        }
      } : undefined,
