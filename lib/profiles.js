  ...
  while (!quit) {
    ...
    await util.choose({rl, us}, {
      ...
      A: profile.username === us.username ? {
        ...
        action: async () => {
          const newValue = await util.prompt(rl, 'New about me: ', profile.aboutMe)
          if (newValue !== profile.wiwo) {
            await util.prettyFetch({}, `${util.urls.siteAPI}/users/all/${profile.username}/`, {
              method: 'PUT',
              body: JSON.stringify({bio: newValue}),
              headers: util.makeFetchHeaders(us)
            })
            profile.aboutMe = newValue
          }
        }
      } : undefined,
      ...
      W: profile.username === us.username ? {
        ...
        longcodes: ['change-about'],
        action: async () => {
          const newValue = await util.prompt(rl, 'New what I\'m working on: ', profile.wiwo)
