...
module.exports.browse = async function({rl, us}) {
  ...
  await pagedList.browse({
    rl, us,
    getItems: async n => (await module.exports.get(us.username, token, n)),
    ...
    handleItem: async m => {
      ...
      await util.choose({rl, us}, {
        ...
        a: m.actor_username === 'systemuser' ? undefined : {
          ...
          action: async () => {
            await profiles.browse({rl, us}, await profiles.get(m.actor_username))
          }
        },
        ...
        p: m.project_id ? {
          ...
          longcodes: ['project'],
          action: async () => {
            await projects.browse({rl, us}, await projects.get(m.project_id))
          }
        } : undefined,
        ...
        g: m.comment_obj_id ? {
          ...
          action: async () => {
            if (m.comment_type === 0) {
              await projects.browse({rl, us}, await projects.get(m.comment_obj_id))
            } else if (m.comment_type === 1) {
              await profiles.browse({rl, us}, await profiles.get(m.comment_obj_title))
            } else {
              // TODO: Studio view.
              console.log('Sorry, studio views aren\'t implemented yet! ' + util.smile())
              await new Promise(res => setTimeout(res, 800))
            }
          }
        } : undefined
...
module.exports.get = function(username, token, pageNum = 1) {
