...
module.exports.browse = async function({rl, us}) {
  ...
  const showDetails = function() {
    console.log("[all: fp] Latest featured project: [FP ->]", proj(FP))
    console.log("[all: fs] Latest featured studio: [FS ->]", studio(FS))
    console.log("[all: cp] Latest curated project: [CP ->]", proj(CP))
    console.log("[all: sds] From the Scratch Design Studio: [SDS ->]", proj(SDS))
    console.log("[all: cr] What the community is remixing: [CR ->]", proj(CR))
    console.log("[all: cl] What the community is loving: [CL ->]", proj(CL))
    console.log("[all: rp] A recent project: [RP ->]", proj(RP))
    console.log("[all: bf] Made by a Scratcher you're following: [BF ->]", proj(BF))
    console.log("[all: lf] Loved by a Scratcher you're following: [LF ->]", proj(LF))
    console.log("[all: sf] From a studio you're following: [SF ->]", proj(SF))
  }
  ...
  const listEntries = function(items, title) {
    return list.browse({
      ...
      formatItem: x => x.type === 'project' ? proj(x) : studio(x),
      handleItem: x => (x.type === 'project'
