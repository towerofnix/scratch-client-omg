...
module.exports.browse = async function({rl, us, id}) {
  ...
  while (!quit) {
    ...
    console.log(`\x1b[2m${project.isRemix ? 'Remixed' : 'Created'} by \x1b[34;2;1m${project.author}\x1b[0;2m; id: ${id}\x1b[0m`)
    if (project.isRemix) {
      console.log(`\x1b[2mOriginal project: \x1b[33;2;1m${project.originalProject.title}\x1b[0m`)
    }
    if (project.tags.length) {
      console.log(`\x1b[2mTagged ${project.tags.map(t => `"${t}"`).join(', ')}\x1b[0m`)
    }
...
function parseProject(html) {
  ...
  const project = {
    ...
    tags: $('#project-tags .tag').map((i, tagElement) => {
