...
module.exports.browse = async function({rl, us, id, jumpToComment = null}) {
  ...
  const showNotes = function() {
    if (project.instructions) {
      ...
      console.log(util.wrap(project.instructions))
      console.log('')
    }

    if (project.notesAndCredits) {
      console.log('\x1b[1mNotes and Credits:\x1b[0m')
      console.log(util.wrap(project.notesAndCredits))
      console.log('')
    }
  }
  ...
  while (!quit) {
    ...
    console.log(header)
    if (project.isRemix) {
      console.log(`\x1b[2mBased on: \x1b[33;2;1m${project.remixParent.title}\x1b[0m`)
    }
    ...
    await util.choose({rl, us}, {
      ...
      R: project.isRemix ? {
        help: `View the project this was remixed upon, \x1b[33;1m${project.remixParent.title}\x1b[0m.`,
        longcodes: ['remix'],
        action: async () => {
          await module.exports.browse({rl, us, id: project.remixParent.id})
        }
      } : undefined,
  ...
  return fetch(`${util.urls.newAPI}/projects/${id}`)
    ...
    .then(data => {
      ...
      const loadRemixData = () => {
        ...
        return fetch(`${util.urls.newAPI}/projects/${data.remix.parent}`)
          .then(res => res.json())
          .then(data => {
            project.remixParent = {
              id: data.id,
      ...
      const loadTagData = () => {
        ...
        return fetch(`${util.urls.siteAPI}/tags/project/${id}`).then(
          res => res.json().then(data => {
            project.tags = data.map(entry => entry.fields.tag.name)
          }),
          err => null // Probably a 404, no big deal.
        )
      }
